var table = {
	...
	create: function() {
		...
		var table = d3.select("table#catTable");
		...
		function filterByName(list) {
			...
			list = list.filter(function (cat) {
				return cat.name === allCatalogueName || cat.name.toLowerCase().indexOf(filterName) !== -1;
			});
			return list;
		}
		...
		// Column ordering
		d3.selectAll("#catTable th").on("click", function( pushed ) {
